...
def comunicacion():         # Funcion que va leyendo los datos que llegan de la arduino
    ...
    if ready[0]:                                        # Verifica si llego algun dato de la arduino        
        leido += arduino.recv(1024).decode()            # Como los datos a veces llegan cortados, se van acumulando todos los que llegan para despues organizarlos por lineas
...
def comando(boton, comand):     # Funcion que envia el comando a la arduino
    ...
    thread.start_new_( checkComando, (estadoAnterior, comand, boton, ) )
...
def conectarServidor():
    _thread.start_new_thread(conectar, ())
...
def recibirComandos():  
    ...
    try:    
        ...
        conectar()
    _thread.start_new_thread(recibirComandos, ())
...
def enviarAltura(boxAltura, boxAltura1):
    ...
    _thread.start_new_thread( checkEnvioALtura, (alturaNueva, alturaNueva1,) )
